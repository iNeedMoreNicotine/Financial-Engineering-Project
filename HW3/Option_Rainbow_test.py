...
def cov_matrix(n, sigmas, rhos):  # n-dimension
    cov_matrix = []
    for i in range(n):
        row = [0] * n
    ...
    for i in range(n):
        cov_matrix[i][i] = sigmas[i]**2

    for i in range(n-1):
        for j in range(i, n):
            cov_matrix[i][j] = sigmas[i]*sigmas[j]*rhos[f'rho{i+1}_'][j-1]
...
# sigmaDict = dict()
# for i in range(n):
#     sigmaDict[f'sigma{i+1}'] = sigmas[i]
# locals().update(sigmaDict)
# for i in range(n):
#     sigmas[i] = sigmas[i] * sqrt(T)

# [[ρ12, ρ13, ρ14], [ρ23, ρ24], [ρ34]]
# rhos = dict()
# for i in range(n-1):
#     rhos[f'rho{i+1}_'] = [0]*(n-1)
#                  2    3    4    5
# rhos['rho1_'] = [0.5, 0.5, 0.5, 0.5]
# rhos['rho2_'] = [0,   0.5, 0.5, 0.5]
# rhos['rho3_'] = [0,   0,   0.5, 0.5]
# rhos['rho4_'] = [0,   0,   0,   0.5]
